...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header)
{
    ...
    if (solver_flag == INITIAL)  // 需要初始化
    {
        if (frame_count == WINDOW_SIZE) // 滑动窗口中塞满frame了才进行初始化
        {
            ...
            if(!STEREO && ESTIMATE_EXTRINSIC != 2 && (header - initial_timestamp) > 0.1)  // 在上一次初始化失败后至少0.1秒才进行下一次初始化
            {
               result = initialStructure();
               initial_timestamp = header;
            }
            if(STEREO || result) // 初始化操作成功
            {
                ...
                f_manager.removeOutlier(removeIndex);    // 去除滑出了滑动窗口的特征点
                if (! MULTIPLE_THREAD)
                {
...
bool Estimator::initialStructure()
{
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        if((frame_it->first) == Headers[i]) // all_image_frame与滑动窗口中对应的帧
        {
...
bool Estimator::visualInitialAlign()
{
    ...
    for (int i = 0; i <= frame_count; i++)
    {
       // 滑动窗口中各图像帧在世界坐标系下的旋转和平移
        ...
        Ps[i] = Pi;
        Rs[i] = Ri;
        all_image_frame[Headers[i]].is_key_frame = true;  // 滑动窗口中所有初始帧都是关键帧
    }
    ...
    Matrix3d R0 = Utility::g2R(g); //当前参考坐标系与世界坐标系（依靠g构建的坐标系）的旋转矩阵 TODO 谁转谁
...
// 后端优化后的变量更新：
// 其中rot_diff是根据窗口中第一帧在优化前后的yaw偏差计算得到的旋转矩阵，以后对滑窗中第一帧在优化前后的
// yaw偏差计算得到的矩阵旋转，以后对滑窗内的所有帧都进行rot_diff的校正。这是因为在后端滑动窗口的非线性优化时，
// 并没有固定住第一帧的位姿不变，而是将其作为优化变量进行调整。但是，因为相机的偏航角yaw是不可观测的，也就是对于
// 任意的yaw的满足优化目标函数，因此优化之后我们将航偏角旋转至优化前的初始状态。
...
void Estimator::optimization()
{
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        TicToc t_pre_margin;
// 得到每次IMU和视觉观测（cost_function）对应的参数块（parameter_blocks）,雅可比矩阵（jacobians）,残差（residuals）
