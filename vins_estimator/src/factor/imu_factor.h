...
// 7是第k时刻的位置和旋转，9是第k时刻的速度，加速度计和陀螺仪的bias;之后的是k+1时刻
...
//delta_q = Qi.inverse() * Qj;
        ...
        residual = pre_integration->evaluate(Pi, Qi, Vi, Bai, Bgi,
                                            Pj, Qj, Vj, Baj, Bgj);
// residual乘以一个信息矩阵sqrt_info，这是因为真正的优化项其实是马式距离;又因为ceres只接受最小二乘优化，所以把斜方差P的逆进行分解。马式距离其实相当于一个残差加权，
// 斜方差小的加权大，着重优化那些比较确定的残差。若写成sqrt_info.setIdentity()就是不加权。
